...
export default function MarketAccessPage({ locale }: { locale: Locale }) {
  ...
  return <main className="service-page" lang={c.lang}>
    ...
    <section className="service-related dark-section"><div className="shell section-pad"><p className="section-index">{locale === "en" ? "EXPLORE OUR EXPERTISE" : "CONOCE NUESTROS SERVICIOS"}</p><a className="service-solution-callout" href={locale === "en" ? "/solutions/healthcare-market-entry-mexico" : "/es/soluciones/entrada-mercado-healthcare-mexico"}><span>{locale === "en" ? "FEATURED SOLUTION" : "SOLUCIÓN DESTACADA"}</span><strong>{locale === "en" ? "Healthcare Market Entry & Growth Strategy in Mexico" : "Estrategia de Entrada y Crecimiento Healthcare en México"}</strong><i className="icon-arrow icon-ne" aria-hidden="true" /></a><div className="service-related-list">{related.map(([label, href], index) => <a href={href} key={href} aria-current={href === pagePath ? "page" : undefined}><span>{String(index + 1).padStart(2, "0")}</span>{label}<i className="icon-arrow icon-ne" aria-hidden="true" /></a>)}</div></div></section>
