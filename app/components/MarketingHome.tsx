...
export default function MarketingHome({ locale }: { locale: Locale }) {
  ...
  return <main lang={c.lang}>
    ...
    <section className="services shell section-pad" id="servicios"><header className="section-head"><p className="section-index">{c.expertise.index}</p><p className="kicker">{c.expertise.kicker}</p></header><div className="service-list">{c.expertise.services.map((service, index) => <article className="service-card" key={service.title[0]}><span className="number">{String(index + 1).padStart(2, "0")}</span><div><h3>{service.title[0]}<br />{service.title[1]}</h3><p>{service.body}</p><ul>{service.items.map((item) => <li key={item}>{item}</li>)}</ul></div><span className="card-arrow icon-arrow icon-ne" aria-hidden="true" /></article>)}</div></section>
